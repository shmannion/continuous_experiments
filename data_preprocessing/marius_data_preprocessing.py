...
def get_trials_conditions(raw_data, pair):
    ...
    marker_vals = raw_data['Marker'].unique()
    print(f'we have the following marker vals: {marker_vals}')
...
def assign_new_columns_and_save(df, pair, participant):
    ...
    for condition_no in cond_vals:
        ...
        print(outdir)
        condition_df = df[df['condition'] == condition_no]
        ...
        print(f'for condition {condition_no}, there are {len(trial_values)} trials, df_map has len {len(df_map)}')
        lookup = df_map.set_index('old_trial_no')['new_trial_no']
        condition_df['new_trial_no'] = condition_df['trial_no'].map(lookup)
        ...
        for trial_no in trial_file_labels:
            ...
            print(f'saving to {outdir}, pair {pair}')
            # print(f'trial dataframe for participant {participant}, {participant_label}')
            # print(trial_df.head())
            trial_df.to_csv(f"{outdir}/p_{pair:02d}{participant_label}_trial_{trial_no:02d}.csv", index=False)


if __name__ == "__main__":
    for pair in range(3, 11):
        pp1, pp2 = load_raw_data(pair)
        assign_new_columns_and_save(pp1, pair, 1)
        assign_new_columns_and_save(pp2, pair, 2)
